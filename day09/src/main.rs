...
fn update_tail_history(tail_pos: &Point, tail_history: &mut Vec<Point>) {
...
fn rope_move(dir: &str, dist: i32, pos: &mut Point, tail_pos: &mut Point, tail_history: &mut Vec<Point>) {
    let mut delta = Point{
        x: 0,
        y: 0,
    };
    ...
    for _ in 0..dist {
        ...
        update_tail_history(&tail_pos, tail_history);
    }
}
...
fn main() {
    ...
    update_tail_history(&tail_pos, &mut tail_history);
    loop {
        ...
        rope_move(dir, dist, &mut pos, &mut tail_pos, &mut tail_history);
    }
